...
const { Formattings, ChatColors } = require("@greenfrog/mc-enums")
...
function generate_fail_message(error) {
	const message =
			`
				${ChatColors.Red}Failed to start the server
				${error.stack}
...
async function initialize_server() {
	if (!fs.existsSync(DEFAULT_CONFIG_NAME)) {
		const config_path = path.join(__dirname, ".", "src", "resources", "yml", "default_config.yml")
