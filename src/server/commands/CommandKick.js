...
class CommandKick extends require('./Command') {
    ...
    execute(args) {
        if (!commands.console_command_kick) { Logger.log(lang.unknownCommand); return; }
        if (!args) {
        ...
        for (let i = 0; i < PlayerInfo.prototype.getPlayers().length; i++) {
            ...
            if (client.username == args[0]) {
                ...
                playeroffline = false
                Logger.log(lang.kickedConsoleMsg.replace('%args[0]%', args[0]).replace('%args[1]%', args[1]), 'info')
            }
        }
