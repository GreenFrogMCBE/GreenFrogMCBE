...
class CommandOp extends require('./Command') {
    ...
    execute(args) {
        if (!commands.console_command_op) { Logger.log(lang.unknownCommand); return }
        if (!args) { Logger.log(lang.commandUsageOp); return }
    ...
    executePlayer(client, args) {
        ...
        if (!args.split(" ")[1]) { client.sendMessage('§c' + lang.commandUsageOp); return }

        fs.appendFile('ops.yml', args.split(" ")[1] + '\n', (err) => {
            if (!err) client.sendMessage(lang.opped.replace('%player%', args.split(" ")[1]))
            else {
