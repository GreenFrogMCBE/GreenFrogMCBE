...
class CommandVersion extends require('./Command') {
    ...
    execute() {
        if (!commands.console_command_version) { Logger.prototype.log(lang.unknownCommand); return; }
        Logger.log(lang.commandVerInfo.replace("%version%", ServerInfo.serverversion))
    }
    ...
    executePlayer(client) {
        ...
        client.sendMessage(lang.playerVerCommandLine1.replace('%version%', ServerInfo.serverversion))
