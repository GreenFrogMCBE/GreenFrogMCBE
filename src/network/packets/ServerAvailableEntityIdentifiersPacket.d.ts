export = ServerAvailableEntityIdentifiersPacket;
declare class ServerAvailableEntityIdentifiersPacket extends PacketConstructor {
    /**
     * Sets the packet value
     * @param {JSON} new_value - The packet value data to set.
     */
    setValue(new_value: JSON): void;
    /**
     * Returns the packet value
     * @returns {String} The packet value.
     */
    getValue(): string;
    /**
     * Sends the packet to the client
     * @param {any} client
     */
    writePacket(client: any): void;
}
import PacketConstructor = require("./PacketConstructor");
