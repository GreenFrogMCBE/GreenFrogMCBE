...
class Transfer extends require("./Packet") {
    ...
    validate(client, address, port) {
        ...
        if (!address) throw new Error("Packet processing error. Target server address is null")
        if (parseInt(port) == NaN) throw new Error("Packet processing error. A plugin tried to sent a player to invalid server (port must be int, not string)")
    }
