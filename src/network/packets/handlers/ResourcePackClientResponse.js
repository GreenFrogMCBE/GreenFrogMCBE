        ...
        switch (packet.data.params.response_status) {
            ...
            case 'completed': {
                if (client.username.includes(" ")) {
                    new Logger().log(lang.invalidUsernameWarning.replace('%player%', client.username), "warning")
                ...
                for (let i = 0; i < ops.length; i++) {
                    if (ops[i] == client.username) { client.op = true; client.permlevel = 4 }
                    else { client.op = false; client.permlevel = config.default_permission_level }
                }

                Logger.log(lang.joined.replace('%player%', client.username))
                new PlayerList().writePacket(client, client.username)
                new StartGame().writePacket(client, config.gamemode, config.world_gamemode, client.permlevel)
                ...
                setInterval(() => {
                    if (!client.offline) new NetworkChunkPublisherUpdate().writePacket(client)
                }, 50)

                Logger.log(lang.spawned.replace('%player%', client.username))
                setTimeout(() => {
                    if (!client.offline) new PlayStatus().writePacket(client); new Events().executeOPS(server, client)
                }, 2000)

                setTimeout(() => {
                    if (client.offline) return
                    for (let i = 0; i < new PlayerInfo().getPlayers().length; i++) {
                        new PlayerInfo().getPlayers()[i].sendMessage(lang.joinedTheGame.replace('%username%', client.username))
                    }
                }, 1000)
                break
            }
            default: {
                if (config.logunhandledpackets) Logger.log(lang.unhandledPacketData.replace('%data%', packet.data.params.response_status))
            }
        }
    }
