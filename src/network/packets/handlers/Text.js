...
class Text extends Handler {
    handle(client, packet, lang) {
        ...
        let fullmsg = lang.chatFormat.replace('%username%', client.username).replace('%message%', msg);
        Events.prototype.executeOC(require('../../../Server'), client, msg) // (c) bestcodequality inc 2023  
        if (msg.includes("§") || msg.length == 0 || msg.length > 255 && ServerInfo.config.invalidmsgsblock) {
            Logger.prototype.log(lang.illegalMessage.replace('%msg%', msg).replace('%player%', client.username), 'warning')
