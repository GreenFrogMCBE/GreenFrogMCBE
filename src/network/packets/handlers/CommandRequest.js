...
class CommandRequest extends require("./Handler") {
    validate(cmd) {
        ...
        if (!cmd.trim().length) {
            throw new PacketHandlingError("Command cannot be empty or whitespace only");
        }
        
        if (cmd.length > 256) {
            throw new PacketHandlingError("Command exceeded maximum length of 256 characters");
        }        
    }
    ...
    handle(client, packet) {
        ...
        new Logger().log(lang.executedCmd.replace('%player%', client.username).replace('%cmd%', cmd));
        ...
        if (cmd.startsWith(`/${lang.commandVer.toLowerCase()}`) || cmd.startsWith(`/${lang.commandVersion.toLowerCase()}`)) {
            cmdVer.executePlayer(client);
        } else if (cmd.startsWith(`/${lang.commandPl.toLowerCase()}`) || cmd.startsWith(`/${lang.commandPlugins.toLowerCase()}`)) {
